...
def analyze(fp):
    ...
    while not IsEof(fp):
        ...
        if m:
            ...
            tagDataUntil(fp, b'\x0a', None, 'DATA %s: %s' % (addr, data))
            continue

        m = re.match(r'^:00000001FF', line)
        if m:
            tagDataUntil(fp, b'\x0a', None, 'EOF')
            continue

        m = re.match(r'^:02(....)02(....)(..)', line)
        if m:
            (addr, saddr, csum) = m.group(1,2,3)
            tagDataUntil(fp, b'\x0a', None, 'EXTENDED SEGMENT ADDR %s' % (saddr))
            continue

        m = re.match(r'^:04(....)03(....)(....)(..)', line)
        if m:
            (addr, cs, ip, csum) = m.group(1,2,3,4)
            tagDataUntil(fp, b'\x0a', None, 'START SEGMENT ADDR %s:%s' % (cs, ip))
            continue

        m = re.match(r'^:02(....)04(....)(..)', line)
        if m:
            (addr, upper16, csum) = m.group(1,2,3,4)
            tagDataUntil(fp, b'\x0a', None, 'EXTENDED LINEAR ADDR %s0000' % (csum))
            continue

        m = re.match(r'^:04(....)05(........)(..)', line)
        if m:
            (addr, linear, csum) = m.group(1,2,3,4)
            tagDataUntil(fp, b'\x0a', None, 'START LINEAR ADDR %s' % (linear))
            continue
