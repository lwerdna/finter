...
def seek_nalu(fp):
    ...
    if buffer[0:3] == b'\x00\x00\x01':
        ...
        raise Exception(f'expected start code at 0x{o_code:X}')

    o_nalu = o_end = o_code + len(code)
    fp.seek(o_nalu)

    while True:
        chunk = fp.read(4096)

        # found! end is offset within buffer
        i = chunk.find(code)
        if i != -1:
            o_end += i
            break
        ...
        # back
        o_end -= len(code)
        fp.seek(-len(code), io.SEEK_CUR)

    # back home
    fp.seek(o_code, io.SEEK_SET)
    return (len(code), o_end - o_nalu)
